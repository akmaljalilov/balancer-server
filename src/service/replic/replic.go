...
func (h *MysqlGTIDHandler) WaitRelayLogDone(s *Server) error {
	...
	res, err := s.SlaveStatus()
	if err != nil {
		return errors.Trace(err)
	}

	retrieved := res["Retrieved_Gtid_Set"].String

	// may only support MySQL version >= 5.6.9
	// see http://dev.mysql.com/doc/refman/5.6/en/gtid-functions.html
	return h.waitUntilAfterGTIDs(s, retrieved) // TODO
}

func (h *MysqlGTIDHandler) WaitCatchMaster(s *Server, m *Server) error {
	_, err := m.MasterStatus()
	if err != nil {
		return errors.Trace(err)
	}

	//masterGTIDSet, _ := r.GetStringByName(0, "Executed_Gtid_Set") //TODO

	return h.waitUntilAfterGTIDs(s, "masterGTIDSet") //TODO
}
